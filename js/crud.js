...
let elChooseInput = document.querySelector(".form-input")
...
function renderTodos(arr, list){
    ...
    arr.forEach ((item,index) => {
        ...
        <div onclick="handleCheckClick(${item.id})"  id="complate"  class="w-[20px] relative flex items-center justify-center h-[20px] rounded-full border-[1px]  border-slate-500">
        <div id="complate" class="${item.isComplated ? "bg-red-900" : ""} absolute inset-[1.2px] mx-auto rounded-full "></div>
        </div>
        </label>
            ...
            <div class = "flex items-center gap-2"> 
                <button id="edit" onclick="handleEditBtn(${item.id})" class="w-[100px] h-[30px] bg-blue-500 hover:bg-blue-600 text-white rounded-[5px]">Edit</button>
                <button onclick="handleDeleteBtn(${item.id})" class="w-[100px] h-[30px] bg-red-500 hover:bg-red-600 text-white rounded-[5px]">Delete</button>
            </div>
        </div>
        <img class = "mt-5 ${item.image.includes("null") ? "hidden" : "" } rounded-md h-[300px]" src=${item.image} alt="todo img" />
        `
...
elChooseImg.src = URL.createObjectURL(e.target.file[0])
